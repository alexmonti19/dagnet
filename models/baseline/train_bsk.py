...
from models.baseline.evaluate_bsk import evaluate
...
def main(args):
    ...
    # ONLY WHEN USING SBATCH: check if saves_dir already contains a checkpoint, this means that the job has been killed
    # and restarted -> we can resume from the checkpoint to avoid restarting from scratch
    if list(pathlib.Path(save_dir).glob('*.pth')).__len__() != 0:
